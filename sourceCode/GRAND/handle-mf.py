...
def build_dataset():
    ...
    for dir in os.listdir(cand_dir):
        ...
        with open(cand_path, 'r') as f:
            for line in f:
                if re.match(r"site", line):
                    ...
                    nei_list.sort()
                    if src_name in used and used[src_name] == nei_list:  # 如果已经A->B的A已经出现过，并且两者nei相同
                        if (node_to_node not in true_fault) or judge(index, candidates,
                                                                     node_to_node):  # 如果现在的不是ans，直接排除
                            # print(node_to_node)
                            repeat = True
                        else:  # 否则把之前的排除
                            fake_ans_index = 0
                            for i in range(len(candidates)):  # 先找到之前的下标
                                if candidates[i].cand_id.split('-')[0] == src_name and nei_list == candidates[
                                    i].neighbors:
                                    fake_ans_index = i
                                    break
                            candidates.pop(fake_ans_index)  # 然后把它删除
                            for ip in range(len(index)):  # 如果在后面，index要往前移一位
                    ...
                    if incoh > 0 or val > 0:
                        ...
                        # if node_to_node in ori_candidates:
                        #     print('----------------------------------------',node_to_node,'----------------------------------------')
                        for nei in nei_list:
                    ...
                    if len(index) >= 1 and index[-1] >= len(candidates):
                        # print('something wrong',index,len(candidates))
                        index.pop()
        ...
        if not got_ans:
            no_cand_file_num += 1
            continue
        if len(candidates) / len(index) < args.num:
            less_than_1 += 1
            continue
        ...
        cur_node_feat, no_dep_feat = embedding(cur_node_feat, candidates, node_dep, dict, train_mask, label_mask0)
    ...
    print("exceptions: ", no_file_num, no_cand_file_num, less_than_1, only_1_class)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='b17-ssl', help="choose dataset")
