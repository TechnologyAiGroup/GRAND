    ...
    for i in range(cur_id):
        for ip in inputs:
            if i in all_pairs_dis[ip]:
                no_virtual_ctrl[i] = max(no_virtual_ctrl[i], all_pairs_dis[ip][i])
        for op in outputs:
            if op in all_pairs_dis[i]:
                no_virtual_obv[i] = min(no_virtual_obv[i], all_pairs_dis[i][op])
...
if __name__ == '__main__':
    ...
    cand_dir = os.path.join(root, 'toolB', fault, 'data', args.dataset + '-candidates/')
    ans_dir = os.path.join(root, 'toolB', fault, 'data', args.dataset + '/')
